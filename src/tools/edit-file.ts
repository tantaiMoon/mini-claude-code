...
import type { EditFileArgs, ToolRunner } from '../types.js'

export class EditFile implements ToolRunner<EditFileArgs> {
  // 对工作区内文件做一次精确文本替换，适合小范围补丁式编辑。
  async run({ path: relativePath, oldText, content }: EditFileArgs): Promise<string> {
    try {
