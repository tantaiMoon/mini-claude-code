...
import { backgroundProcess, CONFIG, getErrorCode, getErrorMessage, WORK_DIR } from '../utils/index.js'
import type { RunCommandArgs, ToolRunner } from '../types.js'
...
export class RunCommand implements ToolRunner<RunCommandArgs> {
  // 统一入口：校验命令、处理后台任务控制命令，再决定同步或后台执行。
  async run({ commandLine }: RunCommandArgs): Promise<string> {
    try {
      ...
      return `执行异常：${ getErrorMessage(e) }`
    }
  }

  runBlockingCommandLine(commandLine: string): string {
    ...
    if (getErrorCode(result.error) === 'ETIMEDOUT') {
      return `同步命令超时：${ commandLine }`
    }
    if (result.error) {
  ...
  async runBackgroundCommandLine(commandLine: string): Promise<string> {
  ...
  handleBackgroundTaskControllerCommand(commandLine: string): string | null {
    ...
    if (stopMatch) {
      ...
      if (!backgroundProcess.has(pid)) return `没有找到PID： ${ pid }`
      const { child } = backgroundProcess.get(pid)!
      try {
...
function lookLikeLongRunningCommandLine(commandLine: string): boolean {
...
function decodeResultBuffer(buffer: Buffer): string {
  if (!buffer || buffer.length === 0) return ''
  if (process.platform === 'win32') return iconv.decode(buffer, 'cp936')
  return buffer.toString('utf8')
}

function bindStreamsBuffers(readableStream: NodeJS.ReadableStream | null, buffers: Buffer[]): void {
...
function tailTextLine(text: string, lines: number): string {
  if (!text) return ''
  return text.split(/\r?\n/).slice(-lines).join('\n')
}

// 将收集到的 buffers 合并并按平台编码转成字符串。
function buffersToText(buffers: Buffer[]): string {
